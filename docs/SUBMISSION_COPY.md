...
volatility against what a stock actually delivers, buys defined-risk debit spreads only when
...
that clears every liquidity gate, and builds a defined-risk debit spread. Maximum loss is the
premium paid: not a stop that can gap through, but arithmetic that is known before the order
exists. Twenty-seven deterministic gates then run — quote freshness, relative spread, displayed
depth, per-trade loss cap, equity percentage, daily loss budget, portfolio exposure, open
position count, buying power, duplicate client order ID, and more. Claude may narrate and
...
Every run also reviews open positions and exits on whichever comes first: +50% of maximum
profit, -50% of premium paid, or seven days to expiry, before gamma and pin risk dominate.
...
account ID is verified before execution and a kill switch blocks every path.
...
| Fresh Alpaca paper account ID | `«FRESH_PAPER_ACCOUNT_ID»` — must be a NEW account |
